getNearestDyad <- function(prefix = "NuPoPScSc", nuc.size = 147){
	...
	out1 <- nearestDyadPred(dyad.table = dyad.table.unique, 
	...
	out2 <- nearestDyadPred(dyad.table = dyad.table.redundant, 
			chr.num = chr.num, prefix = prefix, nuc.size = nuc.size)
