pred.summary <- function(prefix = "NuPoPScSc"){
	if(length(grep("ScSc", prefix)) == 1 ){
		Target <- "Sc"
		Model <- "Sc"
	}
	if(length(grep("ScSp", prefix)) == 1){
		Target <- "Sc"
		Model <- "Sp"
	}
	if(length(grep("SpSc", prefix)) == 1){
		Target <- "Sp"
		Model <- "Sc"
	}
	if(length(grep("SpSp", prefix)) == 1){
		Target <- "Sp"
		Model <- "Sp"
	}
	if(Target == "Sc"){
	...
	allpred <- get(paste(prefix, ".allpred", sep = ""), envir = .GlobalEnv)
