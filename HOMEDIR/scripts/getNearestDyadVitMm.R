getNearestDyadVitMm <- function(prefix = "NuPoPScSc", nuc.size = 147, mm.chr = "chr19"){
	...
	out1 <- nearestDyadVitMm(dyad.table = dyad.table.unique, 
	...
	out2 <- nearestDyadVitMm(dyad.table = dyad.table.redundant, 
			prefix = prefix, nuc.size = nuc.size, mm.chr = mm.chr)
